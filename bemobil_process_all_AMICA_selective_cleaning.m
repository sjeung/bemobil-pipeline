...
function [ALLEEG, EEG_single_subject_final, CURRENTSET] = bemobil_process_all_AMICA_selective_cleaning(ALLEEG, EEG_interp_avRef, CURRENTSET, subject, bemobil_config, force_recompute)
...
% execute selective cleaning if fields are specified 
if isfield(bemobil_config, 'selective_cleaning')
    selective_cleaning = bemobil_config.selective_cleaning; 
else 
    selective_cleaning = 0; 
end
...
% copy events 
if  isempty(EEG_AMICA_no_eyes.event)
    EEG_AMICA_no_eyes.event = EEG_AMICA_no_eyes.urevent;

    for t=1:length(EEG_AMICA_no_eyes.event)
        EEG_AMICA_no_eyes.event(t).urevent = t;
    end
    
end


%% selective cleaning of continuous data
% output segment indices col1 = starting index, col2 = ending index

% 1) specify segments for cleaning
if selective_cleaning
    
    nSegments           = numel(bemobil_config.selec_segment_names);
    segment_indices     = feval(bemobil_config.selec_segment_function, EEG_AMICA_no_eyes);
    
    for segment = 1:nSegments
        
        EEG_subset(segment) = pop_select(EEG_AMICA_no_eyes, 'point', segment_indices(segment,:));
        EEG_subset(segment) = eeg_checkset(EEG_subset(segment));
        pop_saveset(EEG_subset(segment), 'filename', [bemobil_config.filename_prefix num2str(subject) '_'...
            bemobil_config.selec_segment_names{segment}], 'filepath', output_filepath);
        segments{segment}   = bemobil_config.selec_segment_names{segment}; 

        EEG_subset_filtered(segment) = pop_select(EEG_filtered_for_AMICA, 'point', segment_indices(segment,:));
        EEG_subset_filtered(segment) = eeg_checkset(EEG_subset_filtered(segment));
...
if ~isfield(EEG_AMICA_no_eyes,'auto_continuous_cleaning')
    ...
    automatic_cleaning_settings.chan_select_plot_before_after= [5 40];  %%% [] use all available channels for cleaning
    ...
    %automatic_cleaning_settings.band_artifact_cleaning=[1 100];  %%% in Hz; [] for no filter
    ...
    % loading, cleaning, and saving (results, figures)
    ...
    if selective_cleaning
        ...
        % 2) iterate over segments and clean separately
        
        for segment = 1:nSegments
            
            %%% specify file names
            filename_specifications.file_name_original_EEG = [bemobil_config.filename_prefix num2str(subject) '_' segments{segment} '.set'];   %%% loads "fresh" EEG (raw, unfiltered)
            filename_specifications.filename_saveBadEpochIndices ='';
            
            automatic_cleaning_settings.crit_all = bemobil_config.selec_thresholds(segment); %%% e.g., 0.9=90% keep amount of epochs; 1 value if only 1 file (no appended recordings); else indicate, e.g., 4 values for 4 appended files
            
            disp(['Determining continuous data cleaning boundaries for ' segments{segment} ' set with keep threshold ' num2str(automatic_cleaning_settings.crit_all) '.'])
            ...
            EEG_subset(segment).etc.auto_continuous_cleaning = auto_continuous_cleaning;
            
            disp('Adding buffers to data cleaning boundaries...')
            
            EEG_subset(segment) = add_leading_trailing_samples_FH_cleaning(EEG_subset(segment), bemobil_config.buffer_length);
            
            % reject data from previously computed FH channel data rejection
            % get cleaning indices
            invalid_segments_index = EEG_subset(segment).etc.auto_continuous_cleaning.cleaning_buffer.invalid_segments_final_start_stop_sample_buffered;
            ...
            for freqIndex = 1:numel(bemobil_config.selec_lineNoiseFreq{segment})
                ...
                x = EEG_subset_filtered(segment).data';
                x = nt_demean(x);
                
                % parameters
                FLINE = freq/EEG_subset_filtered(segment).srate; % line frequency
                ...
                EEG_subset_filtered(segment).data = denoised'; 
            ...
            % apply rejection on original data
            EEG_cleaned_for_AMICA_segmented = eeg_eegrej(EEG_subset_filtered(segment), invalid_segments_index);
            
            pop_saveset( EEG_cleaned_for_AMICA_segmented, 'filename', [bemobil_config.filename_prefix num2str(subject) '_'...
                segments{segment} '_cleaned'], 'filepath', output_filepath);
            
            close(gcf);
            
        end
        
        % 3) merge cleaned blocks into one set that enters the AMICA
        ...
        for segment = 1:nSegments
            EEG = pop_loadset('filename',[bemobil_config.filename_prefix num2str(subject) '_' segments{segment} '_cleaned.set'],...
        ...
        [ALLEEG, EEG_cleaned_for_AMICA, CURRENTSET] = bemobil_merge(ALLEEG,EEG,CURRENTSET,1:length(ALLEEG),...
            [ bemobil_config.filename_prefix num2str(subject) '_clean_forAMICA_merged.set'], input_filepath );
        ...
        %%% remove line noise from original data
        % requires noiseTool
        for freqIndex = 1:numel(bemobil_config.selec_lineNoiseFreq)
...
% save RAM
clear EEG_AMICA_raw EEG_subset EEG_cleaned_for_AMICA_segmented
