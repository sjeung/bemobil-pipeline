...
function [ ALLEEG EEG_cleaned CURRENTSET ] = bemobil_reject_continuous(ALLEEG, EEG, CURRENTSET,...
	epochs_length, epochs_overlap, epoch_buffer, fixed_threshold, weights,...
	use_kneepoint, kneepoint_offset, highpass_cutoff, do_plot)
...
elseif ~isfield(EEG.etc, 'filter') || ~isfield(EEG.etc.filter.highpass, 'cutoff')
	...
	[ ALLEEG EEG_for_cleaning CURRENTSET ] =   bemobil_filter(ALLEEG, EEG_for_cleaning, CURRENTSET,...
		100, 10);
    % bemobil_filter(ALLEEG, EEG_for_cleaning, CURRENTSET, highpass_cutoff+1, [],[],[],414);
...
elseif EEG.etc.filter.highpass.cutoff < highpass_cutoff
	...
	disp('This can be disabled by setting the "filter_freq" to 0.')
	[ ALLEEG EEG_for_cleaning CURRENTSET ] = bemobil_filter(ALLEEG, EEG_for_cleaning, CURRENTSET, highpass_cutoff+1, [],[],[],414);
